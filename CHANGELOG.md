...
- 统一物品与界面用词：破碎残片 / 破碎残册 / 破碎编年史 / 失传墨水 / 失传铭刻；书写界面的载体标签、英文界面名一并对齐
  （`Fragment Page` → `Broken Fragment`，`Shard Book` → `Broken Tome`，标签页 `Books & Paper` → `Written Books & Paper`）。
- 收录进度显示默认关闭。
...
- Items: Broken Fragment, Broken Tome, Fragment Chronicle, Lost Ink. Block: Lost Inscription.
...
counter and mod-item recipes are opt-in; untitled entries all read "some words on something"; item and UI names were unified (Broken Fragment, Broken Tome, Fragment Chronicle, Lost Ink, Lost Inscription) and the vanilla tab is "Written Books & Paper".
