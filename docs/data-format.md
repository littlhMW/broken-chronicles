...
- 画布统一**横屏 512×288（16:9）**，所有阅读材质用同一画布尺寸（代码中 `ReadingScreen` 的 `CANVAS_WIDTH/CANVAS_HEIGHT` 常量）。
- 内容画在画布的**非透明区域**：阅读界面按非透明区域**自动裁剪缩放居中**，**非透明区域多大就显示多大**——画满整张画布就是全屏横图；只画一小块（例如 56×56）就显示为小图。
- 透明区域不参与显示，透出深色底；适合做纸片、树叶、边框等自由形状，也可以做全屏背景。
- 文字默认画在非透明区域的中央（左右留 8% 边距、上下约 12%~18% 起），不要把重点画到边缘太外侧。
